...
def view_classtimetable(request):
	if request.method == 'POST':
		cursor=connection.cursor()
		cursor.execute("select day,slot_no_id,cname_id,rid from timetable_timetable;")
		result=cursor.fetchall()
		r=[]
		for i in result:
			u=[]
		...
		return render(request,'timetable/viewclass.html')
	...
	if request.method == 'POST':
		if request.POST.get('c_name'):
			...
			if(len(result)!=0):
				...
				print(r)
				context={'talist':r,'coursename':cname}
				return render(request, 'timetable/student_Ta.html',context)
			else:
				context={'msg':"TA's are not allocated yet"}
...
def schedulefm(request):
 ...
 crsr = connection.cursor()
 if request.method == 'POST':
  if request.POST.get('txtDate') and request.POST.get('purpose') and request.POST.get('type'):
     pr=1;
     print("////////")
